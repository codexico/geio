 ...
 * Modificadas as funcoes "numbers" e "__pagingLink"
 * para passar tambem o parametro id em paginacoes complexas com outros modelos
 *
 ...
 * @subpackage    cake.cake.libs.view.helpers
 */
class PaginacaoHelper extends AppHelper {
/**
 * Helper dependencies
 *
 * @var array
 ...
 */
	function __pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		//$url = array_merge(array('page' => $paging['page'] + ($which == 'Prev' ? $step * -1 : $step)), $url);
                $url = array_merge(array('page' => $paging['page'] + ($which == 'Prev' ? $step * -1 : $step)), $this->params['pass']);
                //debug($url);
		if ($this->{$check}($model)) {
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					$out .= $this->Html->tag($tag, $i, array('class' => 'current'));
                                    ...
                                    if(isset($this->params['pass'][0])){//Passa parametro id tambem
                                        $out .= $this->Html->tag($tag, $this->link($i, array('page' => $i,$this->params['pass'][0]), $options));
                                    }else{
